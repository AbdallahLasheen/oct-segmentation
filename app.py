...
# Setting up the web page title and university branding
st.set_page_config(
...
# Label mapping based on training data classes and RGB colors
LABEL_MAP = [
...
# Primary fluid categories for pathology quantification
...
def analyze_scan(img, model):
    ...
    # Mathematical Model for Fluid Index
...
def get_groq_ai_response(prompt):
    """Connects to Groq LPU (Llama 3.3) for clinical reasoning."""
...
def create_medical_pdf(p_info, dr_name, report_text):
    """Generates an official PDF report branded for AIU."""
    pdf = FPDF()
    pdf.add_page()
    try:
    ...
    # Branded Header for Alamein International University
    pdf.set_font("Arial", 'B', 16)
    pdf.set_x(45)
    pdf.cell(0, 10, "ALAMEIN INTERNATIONAL UNIVERSITY - AIU", ln=True)
    pdf.set_font("Arial", size=10)
    pdf.set_x(45)
    pdf.cell(0, 5, "VisionOCT | Advanced Neural Imaging & AI Diagnostics", ln=True)
    pdf.ln(15)
...
# ── SECTION 5: MAIN UI & APPLICATION FLOW ──
st.markdown(f'<div class="main-header"><h1>🏥 VisionOCT Diagnostic Suite</h1><p>Alamein International University | Neural Segmentation & Interactive AI Copilot</p></div>', unsafe_allow_html=True)
...
# File Uploader for Sequential OCT Scans
...
if uploaded_files:
    ...
    tab1, tab2, tab3 = st.tabs(["📉 Temporal Trends", "🖼️ Scan Gallery", "📑 Official Report"])

    # --- TAB 1: Data Visualization ---
    with tab1:
        st.subheader("Temporal Progression of Retinal Fluid Concentration")
    ...
    with tab2:
        for data in clinical_history:
            with st.expander(f"👁️ View Detailed Analysis: {data['Filename']}"):
                c1, c2, c3 = st.columns([2, 2, 1.2])
                c1.image(data['Original'], caption="Input B-Scan", width='stretch')
    ...
    # --- TAB 3: CLINICAL DOCUMENTATION & AI INTERACTION ---
    with tab3:
        st.subheader("📝 Professional Clinical Documentation System")
        ...
        if st.button("🚀 Generate AI Clinical Draft", type="primary"):
            if not p_name: st.warning("Please fill in the patient name.")
                ...
                with st.spinner("LPU Engine is synthesizing clinical findings..."):
                    ...
                    st.session_state['report_text'] = get_groq_ai_response(f"Draft a formal clinical report for this OCT data:\n{df[cols].to_string(index=False)}")

        # Step 3: Review, Manual Edit & AI Refinement
        if 'report_text' in st.session_state:
            st.markdown("---")
            st.markdown("### ✍️ Step 2: Professional Review & AI Refinement")
            st.info("💡 **Clinical Interaction:** You can manually edit the text box below, or use the Chat Input to ask the AI to update the report for you.")
            
            # Manual Editor: Allows the physician to change text directly
            ...
            # AI Interactive Refinement: Allows chat-based updates
            user_instruction = st.chat_input("Ask the AI to update the report (e.g. 'Translate to Arabic' or 'Make it formal')")
            if user_instruction:
                with st.spinner("AI Assistant is refining the report..."):
                    refine_prompt = f"Current Report: {st.session_state['report_text']}\n\nDoctor Instruction: {user_instruction}\n\nUpdate the report accordingly. Return ONLY the new text."
                    st.session_state['report_text'] = get_groq_ai_response(refine_prompt)
                    st.rerun()

            # Step 4: Final PDF Finalization
            st.markdown("---")
            ...
            with col_pdf:
                # FIXED: Proper binary handling for PDF export
            ...
            with col_prnt:
                if st.button("🖨️ Open Print Preview"):
                    st.markdown(f'<div style="background:white; color:black; padding:40px; border:2px solid #3b82f6; border-radius:15px; font-family:serif; text-align:left;">'
                                f'<h2 style="text-align:center;">Clinical Assessment Report</h2>'
